...
final class PreferenceHelper {
    ...
    private static final String PREF_KEY_APP_LAUNCH_TIMES = "android_app_launch_times";
    ...
    static int getAppLaunchedTimes(Context context) {
        return getPreferences(context).getInt(PREF_KEY_APP_LAUNCH_TIMES, 0);
    }

    static void setAppLaunchedTimes(Context context, int AppLaunchTimes) {
        SharedPreferences.Editor editor = getPreferencesEditor(context);
        editor.putInt(PREF_KEY_APP_LAUNCH_TIMES, (getAppLaunchedTimes(context)+1)*AppLaunchTimes);
        editor.apply();
    }
